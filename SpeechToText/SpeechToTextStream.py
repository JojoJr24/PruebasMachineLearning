...
RATE_PROCESS = 16000
FILE_SAVE_PATH = "/mnt/ramdisk/temp.wav"
...
class Audio(object):
    ...
    def __init__(self, callback=None, device=None, input_rate=RATE_PROCESS):
        ...
        self.block_size = int(RATE_PROCESS / float(self.BLOCKS_PER_SECOND))
...
class VADAudio(Audio):
    ...
    def frame_generator(self):
        ...
        if self.input_rate == RATE_PROCESS:
            while True:
                yield self.read()
        else:
...
def main(ARGS):
    ...
    silero_vad, utilsVad = torch.hub.load(repo_or_dir='snakers4/silero-vad',
                                  model=ARGS.silaro_model_name,
                                  force_reload=ARGS.reload)
    (get_speech_ts, _, _, _, _) = utilsVad

    # load sileto VoiceToSpeech
    model, decoder, utils = torch.hub.load(repo_or_dir='snakers4/silero-models',
                                           model='silero_stt',
                                           language='es', device= torch.device('cuda'))
    ...
    for frame in frames:
        if frame is not None:
            ...
            time_stamps = get_speech_ts(audio_float32, silero_vad)

            if len(time_stamps) > 0:
               t1 = Thread(target=writewav, args=[newsound,decoder, model, prepare_model_input, read_batch, split_into_batches])
               t1.start()
            wav_data = bytearray()


def writewav(newsound,decoder, model, prepare_model_input, read_batch, split_into_batches):
    scipy.io.wavfile.write(FILE_SAVE_PATH, RATE_PROCESS, newsound)
    file = glob(FILE_SAVE_PATH)
    batches = split_into_batches(file, batch_size=10)
    input = prepare_model_input(read_batch(batches[0]), device= torch.device('cuda'))
    output = model(input)
    respuesta = ""
...
if __name__ == '__main__':
    ...
    # parser.add_argument('-ts', '--trig_sum', type=float, default=0.25, help="overlapping windows are used for each
    # audio chunk, trig sum defines average probability among those windows for switching into triggered state (
    # speech state)")
    ...
    # parser.add_argument('-N', '--num_steps', type=int, default=4,
    #                    help="nubmer of overlapping windows to split audio chunk into (we recommend 4 or 8)")

    # parser.add_argument('-nspw', '--num_samples_per_window', type=int, default=4000, help="number of samples in
    # each window, our models were trained using 4000 samples (250 ms) per window, so this is preferable value (
    # lesser values reduce quality)")
